...
const style={
  ...
  innerTextContainer:`p-4 text-gray-500`
}
const Dropdown=({title,icon,data})=>{
  return(
    <Disclosure>
      {({open})=>(
      <>
        <Disclosure.Button>
          <div className={style.buttonWrapper}>
            ...
            {open?(
        <BiChevronDown classNmae={style.rightIcon}/>
            ):(<BiChevronUp className={style.rightIcon}/>)}
          </div>
        </Disclosure.Button>
         <Disclosure.Panel>
          <div className={style.innerTextContainer}>{data}</div>
        </Disclosure.Panel>
      </>
      )}
    </Disclosure>
